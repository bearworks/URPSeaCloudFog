...
Shader "Skybox/ProceduralCloud" {
	...
	//              for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		    ...
		    half3 SkyFog(half3 color, float worldPosY, float3 ray, float distance)
		    {
		        ...
		        float offsetYPos = (worldPosY - _SkyFogParams.z);
		        float range = _SkyFogParams.y * 30 / SKY_GROUND_THRESHOLD;
		        float t = max(1e-4, (_WorldSpaceCameraPos.y - offsetYPos) / max(range, 1e-6));
		        t = (1 - exp(-t)) / t * exp(-offsetYPos / max(range, 1e-6));

		        float3 extinction = exp(-distance * _SkyFogParams.x * 1e-5 * beta_t * t);

		        float inSov = dot(ray, _WorldSpaceLightPos0.xyz);
			...
			inline float RaySphereIntersect(float sphereRadius, float origin, float DirY)
			{
				float a0 = sphereRadius * sphereRadius - (origin * origin);
				float a1 = (origin * DirY);
			...
			half4 frag(v2f IN) : SV_Target
			{
...
#if 0
				col.rgb = SkyFog(col.rgb, -_SkyFogParams.y * y * 30, -ray, RaySphereIntersect(_SkyFogParams.y * 30 / SKY_GROUND_THRESHOLD + 607583.3, 607583.3, -ray.y));
#else

			    float3 beta_t = _BetaRsMs.xyz + _BetaRsMs.w + _BetaMa;

				float offsetYPos = (-y * 1e4 - _SkyFogParams.z);
				float range = _SkyFogParams.y * 1e5;
				float t = max(1e-4, (_WorldSpaceCameraPos.y - offsetYPos) / max(range, 1e-6));
				t = (1 - exp(-t)) / t * exp(-offsetYPos / max(range, 1e-6));

				float3 extinction = exp(-_SkyFogParams.x * beta_t * t);
				...
				col.rgb = col.rgb * extinction + (1 - extinction) * inscatter;
#endif
